...
const Support = () => {
  ...
  const fetchComplaints = async () => {
    setLoading(true);
    try {
      const res = await api.get('/associate/complaints');
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post('/associate/complaints', form);
      setForm({ subject: '', message: '' });
      setMsg('Complaint submitted! Our support team will get back to you soon.');
      fetchComplaints();
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm">Raise a query or complaint, track its status</p>
      </div>

      <div className="card max-w-xl">
        <h3 className="font-semibold text-gray-800 mb-4">Raise a New Complaint</h3>
      ...
      <div className="card overflow-x-auto p-0">
        <div className="px-5 py-4 border-b"><h3 className="font-semibold text-gray-800">My Complaints History</h3></div>
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan="3" className="text-center py-10 text-gray-400">No complaints raised yet</td></tr>
            ) : (
              complaints.map((c) => (
