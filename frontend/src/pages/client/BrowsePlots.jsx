...
const BrowsePlots = () => {
  ...
  const fetchPlots = async () => {
    ...
    try {
      ...
      const res = await api.get('/client/browse-plots', { params });
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-gray-500 text-sm">Explore available properties from all sellers</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          plots.map((p) => (
            <Link to={`/client/plots/${p.id}`} key={p.id} className="card space-y-3 hover:shadow-md transition-shadow cursor-pointer block">
              {p.image_url ? (
